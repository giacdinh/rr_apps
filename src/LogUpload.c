...
static int LogUpload_ret = BV_FAILURE;
extern int endpoint_url;
static char Encryption[]= "x-amz-server-side-encryption: AES256";

static int LogUpload_readcallback(void *ptr, size_t size, size_t nmemb, void *stream)
{
    size_t retcode;
    curl_off_t nread;
...
static size_t LogUpload_Header_Response(char *buffer, size_t size, size_t nitems, void *userdata)
{
    if(strstr(buffer, "HTTP"))
    {
        if(strstr(buffer, "200 OK") || strstr(buffer, "100"))
            LogUpload_ret = BV_SUCCESS;
        else
        {
            logger_cloud(buffer);
	    LogUpload_ret = BV_FAILURE;
	}
    }
    return nitems;
}
 
int LogUpload(char *upload_auth, char *upfile, char *filetype)
{
    CURL *curl;
    CURLcode res;
    FILE *upload;
    ...
    sprintf((char *) &file_name,"/odi/log/%s", upfile);
    abs_filename = (char *) &file_name;
    ...
    headers = curl_slist_append(headers, (char *) &Encryption);

    /* get a curl handle */ 
    curl = curl_easy_init();
    if(curl) {

        curl_easy_setopt(curl, CURLOPT_READFUNCTION, LogUpload_readcallback);
	curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, LogUpload_Header_Response);
...
//	logger_cloud("%s: ERROR!!! Size missmatch file: %d  -- upload: %d", 
//				__FUNCTION__, file_info.st_size, upload_bytes);

    return LogUpload_ret;
}
