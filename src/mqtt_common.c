...
int client_config_load(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	...
	if(env){
		...
		if(env){
			...
			logger_cloud("Warning: Unable to locate configuration directory, default config not loaded.\n");
		}
	}
	...
	if(cfg->will_payload && !cfg->will_topic){
		logger_cloud("Error: Will payload given, but no will topic given.\n");
		return 1;
	}
	if(cfg->will_retain && !cfg->will_topic){
		logger_cloud("Error: Will retain given, but no will topic given.\n");
		return 1;
	}
	if(cfg->password && !cfg->username){
		if(!cfg->quiet) logger_cloud("Warning: Not using password since username not set.\n");
	}
#ifdef WITH_TLS
	if((cfg->certfile && !cfg->keyfile) || (cfg->keyfile && !cfg->certfile)){
		logger_cloud("Error: Both certfile and keyfile must be provided if one of them is.\n");
		return 1;
	}
#endif
#ifdef WITH_TLS_PSK
	if((cfg->cafile || cfg->capath) && cfg->psk){
		if(!cfg->quiet) 
		    logger_cloud("Error: Only one of --psk or --cafile/--capath may be used at once.\n");
		return 1;
	}
	if(cfg->psk && !cfg->psk_identity){
		if(!cfg->quiet) logger_cloud("Error: --psk-identity required if --psk used.\n");
		return 1;
	}
#endif
	...
	if(pub_or_sub == CLIENT_SUB){
		if(cfg->clean_session == false && (cfg->id_prefix || !cfg->id)){
			if(!cfg->quiet) logger_cloud("Error: You must provide a client id if you are using the -c option.\n");
			return 1;
		}
		if(cfg->topic_count == 0){
			if(!cfg->quiet) logger_cloud("Error: You must specify a topic to subscribe to.\n");
			return 1;
		}
	}
...
int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	...
	for(i=1; i<argc; i++){
		if(!strcmp(argv[i], "-p") || !strcmp(argv[i], "--port")){
			if(i==argc-1){
				logger_cloud("Error: -p argument given but no port specified.\n\n");
				return 1;
			}else{
				cfg->port = atoi(argv[i+1]);
				if(cfg->port<1 || cfg->port>65535){
					logger_cloud("Error: Invalid port given: %d\n", cfg->port);
					return 1;
				}
			}
			i++;
			...
			if(i==argc-1){
				logger_cloud("Error: -A argument given but no address specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --cafile argument given but no file specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --capath argument given but no directory specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --cert argument given but no file specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --ciphers argument given but no ciphers specified.\n\n");
				return 1;
			}else{
			...
			if(pub_or_sub == CLIENT_PUB){
				...
				if(i==argc-1){
					logger_cloud("Error: -C argument given but no count specified.\n\n");
					return 1;
				}else{
					cfg->msg_count = atoi(argv[i+1]);
					if(cfg->msg_count < 1){
						logger_cloud("Error: Invalid message count \"%d\".\n\n", cfg->msg_count);
						return 1;
					}
				}
				i++;
			...
			if(cfg->pub_mode != MSGMODE_NONE){
				logger_cloud("Error: Only one type of message can be sent at once.\n\n");
				return 1;
			}else if(i==argc-1){
				logger_cloud("Error: -f argument given but no file specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -h argument given but no host specified.\n\n");
				return 1;
			}else{
			...
			if(cfg->id_prefix){
				logger_cloud("Error: -i and -I argument cannot be used together.\n\n");
				return 1;
			}
			if(i==argc-1){
				logger_cloud("Error: -i argument given but no id specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -I argument given but no id prefix specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -k argument given but no keepalive specified.\n\n");
				return 1;
			}else{
				cfg->keepalive = atoi(argv[i+1]);
				if(cfg->keepalive>65535){
					logger_cloud("Error: Invalid keepalive given: %d\n", cfg->keepalive);
					return 1;
				}
			}
			i++;
			...
			if(i==argc-1){
				logger_cloud("Error: --key argument given but no file specified.\n\n");
				return 1;
			}else{
			...
			if(cfg->pub_mode != MSGMODE_NONE){
				...
				logger_cloud("Error: -m argument given but no message specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -M argument given but max_inflight not specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --protocol-version argument given but no version specified.\n\n");
				return 1;
			}else{
				if(!strcmp(argv[i+1], "mqttv31")){
					...
					logger_cloud("Error: Invalid protocol version argument given.\n\n");
					return 1;
				}
				i++;
			}
			...
			if(i==argc-1){
				logger_cloud("Error: --proxy argument given but no proxy url specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
			    logger_cloud("Error: --psk argument given but no key specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --psk-identity argument given but no identity specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -q argument given but no QoS specified.\n\n");
				return 1;
			}else{
				cfg->qos = atoi(argv[i+1]);
				if(cfg->qos<0 || cfg->qos>2){
					logger_cloud("Error: Invalid QoS given: %d\n", cfg->qos);
					return 1;
				}
			}
			i++;
			...
			if(i==argc-1){
				logger_cloud("Error: -t argument given but no topic specified.\n\n");
				return 1;
			}else{
				if(pub_or_sub == CLIENT_PUB){
					if(mosquitto_pub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
						logger_cloud("Error: Invalid publish topic '%s', does it contain '+' or '#'?\n", argv[i+1]);
						return 1;
					}
					cfg->topic = strdup(argv[i+1]);
				}else{
					if(mosquitto_sub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
						logger_cloud("Error: Invalid subscription topic '%s', are all '+' and '#' wildcards correct?\n", argv[i+1]);
						return 1;
					}
					cfg->topic_count++;
			...
			if(i==argc-1){
				logger_cloud("Error: -T argument given but no topic filter specified.\n\n");
				return 1;
			}else{
				if(mosquitto_sub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
					logger_cloud("Error: Invalid filter topic '%s', are all '+' and '#' wildcards correct?\n", argv[i+1]);
					return 1;
				}
			...
			if(i==argc-1){
				logger_cloud("Error: --tls-version argument given but no version specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -u argument given but no username specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: -P argument given but no password specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --will-payload argument given but no will payload specified.\n\n");
				return 1;
			}else{
			...
			if(i==argc-1){
				logger_cloud("Error: --will-qos argument given but no will QoS specified.\n\n");
				return 1;
			}else{
				cfg->will_qos = atoi(argv[i+1]);
				if(cfg->will_qos < 0 || cfg->will_qos > 2){
					logger_cloud("Error: Invalid will QoS %d.\n\n", cfg->will_qos);
					return 1;
				}
			}
			i++;
			...
			if(i==argc-1){
				logger_cloud("Error: --will-topic argument given but no will topic specified.\n\n");
				return 1;
			}else{
				if(mosquitto_pub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
					logger_cloud("Error: Invalid will topic '%s', does it contain '+' or '#'?\n", argv[i+1]);
					return 1;
				}
...
unknown_option:
	logger_cloud("Error: Unknown option '%s'.\n",argv[i]);
	return 1;
}
...
int client_opts_set(struct mosquitto *mosq, struct mosq_config *cfg)
{
	...
	if(cfg->will_topic && mosquitto_will_set(mosq, cfg->will_topic,
		...
		if(!cfg->quiet) logger_cloud("Error: Problem setting will.\n");
		mosquitto_lib_cleanup();
		return 1;
	}
	if(cfg->username && mosquitto_username_pw_set(mosq, cfg->username, cfg->password)){
		if(!cfg->quiet) logger_cloud("Error: Problem setting username and password.\n");
		mosquitto_lib_cleanup();
		return 1;
	}
#ifdef WITH_TLS
	if((cfg->cafile || cfg->capath)
		...
		if(!cfg->quiet) logger_cloud("Error: Problem setting TLS options.\n");
		mosquitto_lib_cleanup();
		return 1;
	}
	if(cfg->insecure && mosquitto_tls_insecure_set(mosq, true)){
		if(!cfg->quiet) logger_cloud("Error: Problem setting TLS insecure option.\n");
		mosquitto_lib_cleanup();
		return 1;
	}
#  ifdef WITH_TLS_PSK
	if(cfg->psk && mosquitto_tls_psk_set(mosq, cfg->psk, cfg->psk_identity, NULL)){
		if(!cfg->quiet) logger_cloud("Error: Problem setting TLS-PSK options.\n");
		mosquitto_lib_cleanup();
		return 1;
	}
#  endif
...
int client_id_generate(struct mosq_config *cfg, const char *id_base)
{
	...
	if(cfg->id_prefix){
		...
		if(!cfg->id){
			if(!cfg->quiet) logger_cloud("Error: Out of memory.\n");
			mosquitto_lib_cleanup();
			return 1;
		}
	...
	if(rc>0){
		if(!cfg->quiet){
			if(rc == MOSQ_ERR_ERRNO){
				logger_cloud("%s: error %d", __FUNCTION__, rc);
			}else{
				logger_cloud("Unable to connect (%d).\n", rc);
			}
		}
		mosquitto_lib_cleanup();
		return rc;
...
static int mosquitto__parse_socks_url(struct mosq_config *cfg, char *url)
{
	...
	if(!strncmp(url, "socks5h://", strlen("socks5h://"))){
		...
		logger_cloud("Error: Unsupported proxy protocol: %s\n", url);
		return 1;
	}
	...
	if(!host){
		logger_cloud("Error: Invalid proxy.\n");
		goto cleanup;
	}
	...
	if(port){
		...
		if(port_int < 1 || port_int > 65535){
			logger_cloud("Error: Invalid proxy port %d\n", port_int);
			goto cleanup;
		}
		free(port);
	}else{
