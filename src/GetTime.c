...
static int GetTime_Response(void *ptr, size_t size, size_t nmemb, void *stream)
{
    ...
    char cmd[64];
    sprintf((char *) &cmd, "/bin/date %s", tem);
...
{
    struct stat st;
    bzero((char *) &ConfigXML_time, 32);
    if(stat("/odi/conf/config.xml", &st))
	logger_cloud("Stat config.xml error");
    else
	sprintf((char *) &ConfigXML_time,"Conf_time: %lu", st.st_mtim.tv_sec);

    logger_cloud("Time: %s", (char *) &ConfigXML_time);   
    return (char *) &ConfigXML_time;
}

char *GetFWVersion()
{
    bzero((char *) &FW_version, 32);
    sprintf((char *) &FW_version, "FW_version: %s", getVersion());
    logger_cloud("Version: %s", (char *) &FW_version);
...
int GetTime()
{
    ...
    bzero((char *) &author, 64);
    sprintf((char *) &author, CLOUD_AUTHORIZE_TIME, p_main_token);
    ...
    headers = curl_slist_append(headers, (char *) &Content);
