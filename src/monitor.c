...
// DENCZEK - removed dynamic memory allocation
...
// DENCZEK - removed dynamic memory allocation, fixed memory leak
...
void add_signal_handler() {
    ...
    logger_debug("Intialization of UART done");
}
...
void *serial_raw_read() {
    ...
    while (thread_run) {
        ...
        if ((serial_port_fd > 0)) {
            int cnt = read(serial_port_fd, (read_ptr + serial_raw_read_cur),
                           (COMM_BUFFER_SIZE - serial_raw_read_cur));
            ...
            if (cnt > 0) {
                if ((serial_raw_read_cur + cnt) >
                        (COMM_BUFFER_SIZE - COMM_MESSAGE_SIZE)) // > size of one command wrap
    ...
    if (serial_raw_read_last != serial_raw_read_cur) {
        ...
        if (((serial_raw_read_cur - serial_raw_read_last) >= COMM_MESSAGE_SIZE)
            ...
            if ((serial_raw_read_buf[serial_raw_read_last] != 0xFF)
                ...
                logger_debug("preemble is not FF %X - %X continue",
                             serial_read_buf[0], serial_read_buf[1]);
            }
            ...
            logger_detailed("soft_read got %c%c%c cur=%d las=%d", serial_read_buf[0],
                    serial_read_buf[1], serial_read_buf[2], serial_raw_read_cur,
...
/**** Returns:  0 in case of success, -1 in casroote of error,
        ...
        if ((serial_raw_read_cur - serial_raw_read_last) >= 7) {
            ...
            if (resync_read > 1) {
                ...
                while (serial_raw_read_buf[serial_raw_read_last] != '\n') {
                    serial_raw_read_last++;
                    logger_debug("Sync Buffer cur %d last %d", serial_raw_read_cur,
                                 serial_raw_read_last);
                    if (serial_raw_read_last == serial_raw_read_cur)
                        break;
                }
...
int available_space() {
    ...
    if (statvfs(ODI_DATA, &stat_buf) == 0) {
        ...
        logger_detailed("Avaiable space: %d",(int)(free_blk));
    } else {
...
int ttymx_action(char *command) {
        ...
        if (write_command_to_serial_port("MUF\r\n")) 
	{
        ...
        return 0;
    } else if (strcasecmp(command, "RCO") == 0) {
...
//DOCKED Handle case for pre_event send ETO to ignore REC switch
        ...
        //        }
        return 0;
    } else if (strcasecmp(command, "CHF") == 0) {
...
int stealth_time_action(int gpio_state) {
    ...
    if (stealth_time_state_save == 1) stealth_time_state_save = 2;
    nstate90 = cstate;

    if (camera_docked) return;
...
void checkLogs() {
    ...
    while ( (log_entry = readdir(ptr_dir) )) {
        if (strcmp(log_entry->d_name, ".") == 0 ||
                ...
                log_entry->d_type == DT_DIR) continue;
        struct stat statbuf;
    ...
    while (1) {  // Delete log files until 50% is remaining
        ...
        if (free_space > 50 || c > log_count-2) break;
	...
	// If trigger during charge with pre_event also try to increment recording counter
        ...
        if (!camera_docked || (camera_docked == 1 && recording == 1 && pre_event > 0)) {
            ...
            if (recording && free_space_cnt > 1200) {
                ...
                logger_info("Space available: %d MB, Battery level: %d%%",
                             dspace, battery_level);
