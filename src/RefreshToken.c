...
/* Set test Global value. At final release this s */
static int RefreshToken_Response(void* ptr, size_t size, size_t nmemb, void* stream)
{
	...
	logger_cloud("access token: %s", (char*)&local_token[0]);

	tem = process_json_data((char*)ptr, "refresh_token", 0);
	memcpy((char*)&l_refresh_token[0], tem, strlen(tem));
	logger_cloud("refresh token: %s", (char*)&l_refresh_token[0]);
...
int RefreshToken(char* main_token, char* refresh_token)
{
	...
	sprintf(refresh_data, "%s%s", (char*)&data[0], refresh_token);
	printf("%s\n", refresh_data);
	...
	curl_global_cleanup();
	strcpy(main_token, (char*)&local_token[0]);
	strcpy(refresh_token, (char*)&l_refresh_token[0]);
