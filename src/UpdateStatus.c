...
int UpdateStatus(char* Auth_token, char* upfile, char* cksum)
{
	...
	if (cksum != NULL)
	{
		sprintf((char*)&Filename[0], "{\"filename\":\"%s\",\"status\":\"UPLOADED\",\"MD5-checksum\":\"%s\"}", upfile, cksum);
	}
	else
	{
		sprintf((char*)&Filename[0], "{\"filename\":\"%s\",\"status\":\"UPLOADED\"}", upfile);
	}
	...
	strcat(Auth_signedurl, (char*)&auth_header[0]);
